    ...
    e.preventDefault()
		...
		Appointment.addLabResult(dataJson).then(response => {
			if (response.status === 200) {
				...
				alert(`Sukses menambah hasil lab untuk pasien bernama ${this.state.pasien.nama} dengan hasil ${this.state.labResult.hasil}`)
			} else {
				this.setState({
					loading: false
				})
				alert(`Gagal menambah hasil lab untuk pasien bernama ${this.state.pasien.nama}`)
			}
		})
 }
